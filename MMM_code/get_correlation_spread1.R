...
get_correlation_spread <- function(df, target, media,opt_dma = NULL){
  ...
  for(i in target){ 
        ...
        plot <-     ggplot(df_corr_long, aes(x=media, y=correlation, fill=media, na.rm = T)) +
           ...
           stat_summary(fun=mean, colour="darkred", geom="point", 
                   shape=18, size=3,show.legend = FALSE,na.rm = T) +
                     ...
                     theme(legend.position="none", axis.text.x = element_text(size=10, angle = 60, hjust = 1, vjust = 1),
    ...
    # Box plot of correlation spread using ggplot2
      ...
      temp_list = list('plot' = plot,'corr_df' = df_corr_means)
