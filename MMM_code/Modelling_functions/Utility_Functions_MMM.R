...
preprocess_MMM <- function(data_XyZ=NULL, data_seasonal=NULL, data_norm=NULL,
  ...
  ##Modified version of get_seasonality to compute the seasonal components using STL decomposition
  ##Reference: https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/stl
  get_seasonality_trend = function(df,col='seasonality'){
    # A custom utility function to get a series of monthly seasonalities
    date_start  = min(df$period)
    tmp_ts      = stats::ts(df$target, freq=freq, start=decimal_date(date_start))
    stl.obj     = stats::stl(tmp_ts,s.window = 'periodic')
    col_num     = ifelse(col=='seasonality',1,2)
    return_col  = stl.obj$time.series[,col_num]
    return(as.numeric(return_col))
  }
  ...
  for(trg in target_var){
    ...
    ls_seasonal[[trg]] = data_seasonal %>%
      ...
      dplyr::group_by(dma_id, dma) %>%
      tidyr::nest() %>%
      dplyr::mutate(seasonality = purrr::map(.x = data,col = 'seasonality', .f = get_seasonality_trend),
                    trend = purrr::map(.x = data,col = 'trend', .f = get_seasonality_trend)) %>%
      tidyr::unnest(cols = c(data, seasonality,trend)) %>%
      dplyr::select(dma_id, dma, period, all_of(freq_col), target, seasonality,trend) %>%
      dplyr::arrange(dma_id, dma, !!rlang::sym(freq_col)) %>%
      dplyr::rename(!!paste('seasonality',trg,sep = '_') := seasonality,
                    !!paste('trend',trg,sep = '_') := trend)
  }
  ...
  #Due to the leap year causing a change in the seasonality values by week, joining the lagged df with seasonality by period as well
  #The method of removing the 53rd week has been shown to change the distribution of seasonality and so using this updated approach instead of removing the 53rd week observation

  df_Z_extra <- df_target_lag %>% dplyr::left_join(df_seasonal, by=c('dma_id','dma','period',freq_col)) %>%
...
make_stan_data_MMM <- function(pp_MMM_ls=NULL, 
  ...
  ##Add 1 week,month or quarter based on the type of input data
  data_frequency = pp_MMM_ls$extra$data_frequency
  time_to_add  = switch(data_frequency, "weekly" = "1 week", "monthly" = "1 month", "quarterly" = "1 quarter")
  
  test_date = ifelse(is.null(test_date), seq.Date(data_all$Last_Date, length=2, by=time_to_add )[2], test_date) %>% lubridate::as_date()
...
evaluate_MMM = function(df_XyZ=NULL, pp_MMM_ls=NULL, standata=NULL, stanfit=NULL, media_positive=FALSE, pooling_type_media="partial",pooling_type_control="partial") {
  ...
  y_fitted_stats_train  = dplyr::bind_rows(y_fitted_dma_train$MCMC_fit_metrics_stat, 
    ...
    dplyr::mutate(group  = factor(group, 
                  ...
                  metric = factor(metric, levels = c("r2", "mape", "rmse", "nrmse", "mae"))) %>%
  ...
  ##Returning the MCMC objects to be used to compute the elasticity and contribution for each sample
  MCMC_objs = list(alpha_K_S = MCMC_alpha_K_S,
   				   beta_media = MCMC_beta_media,
				   beta_base_control = MCMC_beta_base_control)
...
summarise_fitted_y = function(MCMC_draw=NULL, group_variable=NULL) {
  ...
  ### MCMC Fit Metrics
  ##NRMSE - Using min and max of true y values
  ##Reference: https://www.marinedatascience.co/blog/2019/01/07/normalizing-the-rmse/
  MCMC_fit_metrics = MCMC_fit_raw %>% 
    ...
    dplyr::summarise(r2_true         = cor(y_true, .value_true)^2,
                     ...
                     nrmse_true      = rmse_true/(max(y_true)-min(y_true))) %>% 
  ...
  MCMC_fit_metrics_stat = MCMC_fit_metrics %>% 
    dplyr::group_by(group) %>%  
    dplyr::summarise_at(vars(r2_true:nrmse_true), key_statistics) %>% 
