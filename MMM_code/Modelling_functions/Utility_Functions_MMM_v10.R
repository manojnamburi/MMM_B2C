...
preprocess_MMM <- function(data_XyZ=NULL, data_seasonal=NULL, data_norm=NULL,
                           target_var=NULL, media_names=NULL,
                           control_names=NULL, weather_names=NULL, compute_PCA = 'NO',
                           N_lag_target=1, N_lag_media=14, min_var_weather=1,
                           DMA_Infos=FALSE, data_frequency = 'monthly', subs = 'yes',
                           y_percentile = 1, X_percentile = 1,
                           special_norm_media = 'NULL', special_norm_var = 'NULL',
                           media_norm13_var = NULL) {
  ...
  cat("1. Target, 'y'\n")
  cat(" - Normalize 'y' by Subscribers/Non-Subscribers.\n")
  cat(" - Transform Normalized 'y' by Max('y').\n")
  cat(" - Create Lagged 'y'.\n")
  cat(" - Create Seasonality.\n")
  cat("2. Media Variables, 'X'\n")
  cat(" - Normalize 'X' by Subscribers/Non-Subscribers.\n")
  cat(" - Transform Normalized 'X' by MinMax('X').\n")
  cat(" - Create Lagged 'X'.\n")
  ...
  if (special_norm_media == 'NULL') {
    if (subs == 'yes'){norm_var_name  = c('dma', 'sub')}else{norm_var_name  = c('dma', 'non_sub')}
  } else {
    if (subs == 'yes'){norm_var_name  = c('dma', 'sub', special_norm_var)}else{norm_var_name  = c('dma', 'non_sub', special_norm_var)}
  }
  ...
  ###### subs/non_sub
  if (subs == 'yes'){norm_var  = 'sub'}else{norm_var  = 'non_sub'}
  ...
  default_names    = c('dma_id', 'dma', 'period', norm_var)
  ...
  df_target_norm_long  = data_XyZ %>%
    ...
    dplyr::mutate(y_norm = y_value/UQ(rlang::sym(norm_var))) %>%
  ...
  df_media_norm_long  = data_XyZ %>%
    dplyr::left_join(df_dma, by=c('dma')) %>%
    dplyr::left_join(data_norm, by=c('dma')) %>%
    dplyr::select(all_of(default_names), all_of(media_names)) %>%
    dplyr::arrange(dma_id, dma, period) %>%
    tidyr::gather(key=X, value=X_value, -all_of(default_names)) %>%
    # dplyr::mutate(X_norm = X_value/UQ(rlang::sym(norm_var))) %>%
    dplyr::mutate(X_norm = ifelse(X == special_norm_media, X_value/UQ(rlang::sym(special_norm_var)), X_value/UQ(rlang::sym(norm_var)))) %>%
    dplyr::select(dma_id, dma, period, X, X_norm) %>% 
    dplyr::as_tibble()
  ...
  if(is.null(media_norm13_var)){
    ...
    df_minmax_media2  = df_media_norm_long %>%
      dplyr::filter(!(X %in% media_norm13_var)) %>%
      dplyr::group_by(dma_id, dma) %>%
      dplyr::summarise(min_2 = min(X_norm, na.rm=TRUE),
                      max_2 = max(X_norm, na.rm=TRUE))
  }
  ...
  df_media_minmax   = df_minmax_media1 %>% 
    dplyr::inner_join(df_minmax_media2, by = c('dma_id', 'dma')) %>% 
    dplyr::inner_join(df_minmax_media3, by = 'X') %>% 
    tibble::add_column(df_minmax_media4) %>% 
    dplyr::mutate(min_2 = ifelse(X %in% media_norm13_var, min_1, min_2),
                  min_4 = ifelse(X %in% media_norm13_var, min_3, min_4),
                  max_2 = ifelse(X %in% media_norm13_var, max_1, max_2),
                  max_4 = ifelse(X %in% media_norm13_var, max_3, max_4))
...
make_stan_data_MMM <- function(pp_MMM_ls=NULL, 
                               ...
                               var_target=NULL, var_control=NULL, df_geo=NULL,
  ...
  if (pooling_type == "partial" && !is.null(media_priors)) {
    df_media = dplyr::tibble(Media_Mix    = pp_MMM_ls$extra$var_media,
  ...
  fe_media    = pp_MMM_ls$extra$var_media                        # Media variables
  ...
  num_media     = length(pp_MMM_ls$extra$var_media)              # of media variables
  ...
  X_media_train = X_media[train_index, , ]
  X_media_test  = X_media[-train_index, , ]
...
evaluate_MMM = function(df_XyZ=NULL, pp_MMM_ls=NULL, standata=NULL, stanfit=NULL, media_positive=FALSE, pooling_type="partial") {
  ...
  # 6. `pooling_type` is set to `partial`.
  #   - If `pooling_type="partial"`, then 'pop_para_stat_media' is generated.
  ...
  pooling_type = standata$media_priors$Pooling_Type[1]
