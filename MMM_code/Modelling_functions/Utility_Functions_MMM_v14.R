...
preprocess_MMM <- function(data_XyZ=NULL, data_seasonal=NULL, data_norm=NULL,
  ...
  tb_control = tibble("Control Variable" = control_norm_per_capita_var, "Normalization Variable" = target_norm_var)
  ...
  for(trg in target_var){
    ...
    ls_target_lag[[trg]] = data_seasonal %>%
      ...
      replace(., is.na(.), 0) %>%
      dplyr::mutate_at(.vars = vars(lag_names),.funs = ~./UQ(rlang::sym(target_norm_var))) %>% ##Normalizing AR per capita according to "Geo-level Bayesian Hierarchical Media Mix Modeling"
