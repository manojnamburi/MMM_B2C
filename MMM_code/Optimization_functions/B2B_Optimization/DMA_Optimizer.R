...
eval_f0_opt <- function(spend, B, dma, ratio, media_list){
  ...
  #' 
  media = spend*ratio
  ga = ceiling(sum(gross_add_cobyla(media, idxdma=dma, media_list)))
  #message(ga)
  return(- ga )
}
...
optimize_MMM_weekly <- function(idxdma,
  ...
  #'@param dma_budget Input weekly budget for the DMA
  ...
  if(print_dma_opt){
    cat(paste(dma_name,
              ...
              " Solution" , format_money(sum(weekly_optimized_solution_dma),0), 
              sep = " "))
    cat('\n')
    flush.console()
  }
  ...
  if(round(sum(weekly_optimized_solution_dma)) != round(dma_budget)){
    ...
    budget_diff <- dma_budget - sum(weekly_optimized_solution_dma)
    ...
    if(budget_diff>0){
      ...
      ratio_budget_vec <- diff_w_ub[adjustable_media]/sum(diff_w_ub[adjustable_media])
    ...
    if(!is.null(adjustable_media)){
      ...
      adj_factor[adjustable_media] <- budget_diff*ratio_budget_vec
