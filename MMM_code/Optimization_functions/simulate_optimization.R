...
source('Optimization_function.R')
source('DMA_Optimizer.R')
source('Smoothening_function.R')
...
#'It produces an average output and best and worst case scenarios for the quarter level.
#'
...
#'@param var_spend_period Spend period for which the mean spend values by DMA and media will be calculated. Defaults to 'All'. See details for specification.
...
#'@param var_exclude_zero_media Media for which zero weeks will be excluded when calculating the mean spend by media and DMA. Defaults to NULL.
...
#'@param var_warm_start Whether to use the previous budget allocation as a starting point and lower bound to the next optimization point. Available only for simulator and not for custom budget. Default is F 

##Simulator function
simulator <- function(data_XyZ,                                                ##Input data which contains spend columns
                      var_specific_input_budget = NULL,
                      var_approach = 'Mean',
                      var_spend_period = 'All',                                    ##Subset period for quarterly computation
                      ...
                      var_lb_budget_sim = .7,                                      ##Simulation upper and lower bounds
                      var_ub_budget_sim = 1.3,
                      ...
                      var_step_size = NULL,                                      ##Step increment for the budget                      
                      ...
                      #var_n_flight_period = NULL,                                      ##Number of periods of flight media
...
#             constraint_media_vec <- c(constraint_media_vec,complete_excl_media)
#             }
...
##--------------------------------------------Main Code section------------------------------------------------
  ...
  ##Checks for var_spend_period and var_approach
  if(!(var_approach%in%c('Mean','MinMax'))){
    stop(cat("Please use either Mean or MinMax as the approach\n"))
  }
...
#   }
  ...
  ##Get the mean spend data by the selected quarter
   mean_quarterly_data <- data_XyZ %>% 
                      ...
                      dplyr::group_by(dma) %>%
                      dplyr::summarize_at(.vars = vars(spend_cols_vec),.funs = mean) %>%
                      dplyr::ungroup() %>% 
                      dplyr::select(spend_cols_vec) %>% 
                      dplyr::rename_at(spend_cols_vec,~media_names_vec) %>%
                      as.matrix()
   
    ##Obtain the weekly mean data by dma from the quarterly mean data
    mean_spend_data <- sweep(mean_quarterly_data,MARGIN = 2,period_vec,'/')
    mean_spend_data <- data.frame(mean_spend_data)
  ##Exclude zero spend weeks only for the media listed in the param var_exclude_zero_media
  if(!is.null(var_exclude_zero_media)){
    mean_spend_data_no_zero <- data_XyZ %>% 
                      dplyr::filter(qy %in% var_spend_period) %>%
                      dplyr::group_by(dma) %>% 
                      dplyr::select(spend_cols_vec) %>%  
                      ...
                      dplyr::rename_at(spend_cols_vec,~media_names_vec) %>%
                      as.data.frame()
       ##Only replace the flight media mean values for optimization
       mean_spend_data[,var_exclude_zero_media] <- mean_spend_data_no_zero[,var_exclude_zero_media]
  }
  
  ##Save the mean spend data for further use later - For calculation of share of spend for constrained media
  #saveRDS(mean_spend_data,'Mean_spend_data.rds')
  ...
  optimization_ub_mat <- var_optimization_ub * mean_spend_data
    ...
    ##Using the minmax approach to get the optimization bounds by dma and media
    mean_quarterly_specific_data <- data_XyZ %>% 
                                    dplyr::filter(qy %in% var_spend_period) %>%
                                    dplyr::select(quarter,qy,dma,spend_cols_vec) %>%
                                    dplyr::group_by(quarter,qy,dma) %>%
                                    dplyr::summarize_at(.vars = vars(spend_cols_vec),.funs = sum) %>%
    ...
    mean_quarterly_spend <- nbr_period_q*sum(mean_spend_data[id_opt_case,-id_constraint])
    min_quarterly_spend <- var_optimization_lb*sum(min_quarterly_mat[id_opt_case,-id_constraint])
    max_quarterly_spend <- var_optimization_ub*sum(max_quarterly_mat[id_opt_case,-id_constraint])
    var_optimization_lb <- round(min_quarterly_spend/mean_quarterly_spend,2)
    var_optimization_ub <- round(max_quarterly_spend/mean_quarterly_spend,2)
  ...
  #cat(paste('Max SDL budget',max_sdl_budget))
  #cat(paste('Min SDL budget',min_sdl_budget))
  
  
  
  
  
  ##Unrounded Optimization bounds
  unrounded_opt_lb <- sum(optimization_lb_mat[id_opt_case,-id_constraint], na.rm = TRUE)*nbr_period_q
  unrounded_opt_ub <- sum(optimization_ub_mat[id_opt_case,-id_excl], na.rm = TRUE)*nbr_period_q
   
  ###Limits for each budget based on the number of flight weeks
    
  all_on_min_q_budget <- plyr::round_any(sum(optimization_lb_mat[id_opt_case,-id_constraint], na.rm = TRUE)*nbr_period_q, var_step_size,f = ceiling)
  
  all_on_max_q_budget <- plyr::round_any(sum(optimization_ub_mat[id_opt_case,-id_constraint], na.rm = TRUE)*nbr_period_q, var_step_size,f = floor)
  
  flight_off_min_q_budget <- plyr::round_any(sum(optimization_lb_mat[id_opt_case,-id_excl], na.rm = TRUE)*nbr_period_q, var_step_size,f = ceiling)
    
  flight_off_max_q_budget <- plyr::round_any(sum(optimization_ub_mat[id_opt_case,-id_excl], na.rm = TRUE)*nbr_period_q, var_step_size,f = floor)
  
  start_budget <- all_on_min_q_budget 
  end_budget <-   flight_off_max_q_budget
  ...
  if(!is.null(var_specific_input_budget)){
    if((unrounded_opt_lb <= var_specific_input_budget) & (var_specific_input_budget <= unrounded_opt_ub)){
      ...
      stop(cat(sprintf('Please ensure the input budget for %s footprint lies between %s and %s for proper optimization. Or change the input optimization limits and simulation limits.',var_footprint,unrounded_opt_lb,unrounded_opt_ub)))
    }
  }else{
  ...
  ###Run the optimizer code
    ...
    for(scenario in all_scenarios){
      ...
      ##Check for each scenario if the total allotted budget adds up to the input budget
      total_optimized_budget <- plyr::round_any(sum(optimized_budget[id_opt_case,media_names_vec])*nbr_period_q,var_step_size,f = round)
      #optimized_sdl_budget <- round(sum(optimized_budget[-var_id_opt,media_names_vec])*nbr_period_q,-rounding_num)
      if(input_budget!=total_optimized_budget){
        warning(cat(sprintf('The total input %s budget does not match the optimized budget for input budget level:%s and scenario:%s',var_footprint,input_budget,scenario)))
      ...
      output_df <- output_df  %>% 
               dplyr::mutate(dma_optimized_budget = nbr_period_q*rowSums(select(.,media_names_vec)))%>%
 ...
 for(data in before_smoothening_budget_list){
   ...
   plot_data <- data %>%
                ...
                dplyr::mutate(CPMT = Input_quarterly_budget/(!!rlang::sym(target_var)),
  ...
  if(var_do_smoothen){
   ...
   for(scenario in var_names_scenarios){
     ...
     smoothened_data <- smoothened_data %>% 
                       dplyr::mutate(dma_optimized_budget_ps = nbr_period_q*rowSums(select(.,media_names_vec)))%>%
